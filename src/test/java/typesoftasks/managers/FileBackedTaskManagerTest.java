...
class FileBackedTaskManagerTest {
    ...
    void saveAndLoadMultipleTasks() throws Exception {
        ...
        // Проверяем количество задач, эпиков и сабтасков
        assertEquals(2, loaded.getAllTasks().stream().filter(t -> !(t instanceof Epic) && !(t instanceof Subtask)).count(), "There should be 2 tasks");
        ...
        // Проверяем что содержимое совпадает (по title/description)
        assertTrue(loaded.getAllTasks().stream().anyMatch(t -> t.getTitle().equals("Task1") && t.getDescription().equals("Desc1")));
        assertTrue(loaded.getAllEpics().stream().anyMatch(e -> e.getTitle().equals("Epic1") && e.getDescription().equals("EpicDesc1")));
        assertTrue(loaded.getAllSubtasks().stream().anyMatch(s -> s.getTitle().equals("Subtask1") && s.getDescription().equals("SubDesc1")));
    }
    ...
    void saveAndLoadTasksContent() throws Exception {
        ...
        // Проверяем сохранение всех полей задачи
        Task loadedTask = loaded.getAllTasks().stream().filter(t -> t.getTitle().equals("Task42")).findFirst().orElse(null);
        assertNotNull(loadedTask);
        assertEquals("Desc42", loadedTask.getDescription());

        Epic loadedEpic = loaded.getAllEpics().stream().filter(e -> e.getTitle().equals("Epic42")).findFirst().orElse(null);
        assertNotNull(loadedEpic);
        assertEquals("EpicDesc42", loadedEpic.getDescription());

        Subtask loadedSubtask = loaded.getAllSubtasks().stream().filter(s -> s.getTitle().equals("Sub42")).findFirst().orElse(null);
